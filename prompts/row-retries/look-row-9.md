Create Codex v2 pet look row 9 for `xiao-haila` as exactly 8 full-body frames in this order: 000, 022.5, 045, 067.5, 090, 112.5, 135, 157.5.

CRITICAL POST-090 LOCK: after the 090 screen-right profile, poses 112.5, 135, and 157.5 must stay in the same screen-right-facing pose family while the eyes, chin, and head progressively pitch downward toward 180. Never flip or mirror the face to screen-left after 090. In every post-090 pose, keep the visible nose tip and gaze on the image-right side of the head center; reduce the horizontal cue gradually while increasing the downward cue. The sequence must remain one continuous up -> screen-right -> down-right arc with no reversal.

CRITICAL COMPONENT-SEPARATION LOCK: make every figure visibly smaller than the prior attempt and centered inside its own equal-width slot. Leave a continuous pure-green vertical gap at least 4% of one slot width between every pair of neighboring sprites from top to bottom. Hair tips, parasol edges, ribbons, skirt feathers, hands, and feet must never touch or overlap the neighboring pose. All eight silhouettes must be eight independently detectable connected groups.
